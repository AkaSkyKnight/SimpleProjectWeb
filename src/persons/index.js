...
class Persons extends Component {
  ...
  onPersonUpdate = person => {
    ...
    if (personInStore && person.name !== personInStore.name) {
      addAlert(`Pessoa com o nome "${personInStore.name}" foi alterado para o nome:  "${person.name}"!`)
    }
  ...
  renderPerson = person => {
    return (
      <li key={person.objectId} className="list-group-item d-flex justify-content-between align-items-center">
        ...
        <ButtonGroup>
          <Button size="sm" variant="outline-primary" onClick={() => this.onEditClick(person)}>Editar</Button>
          <Button size="sm" variant="outline-danger" onClick={() => this.onDeleteClick(person)}>Deletar</Button>
        </ButtonGroup>
      </li>
    );
  };
  ...
  render() {
    ...
    return (
      <div>
        <div className="mb-2">
          <Button onClick={this.onAddClick}>Adicionar Pessoa</Button>
